...
export function onChange(fn) {
  ...
  document.addEventListener('click', (e) => {
    ...
    const p = params();
    // A plain navigation lands at the top — go there *before* rendering, so the
    // new view is laid out at its final scroll position and the browser paints
    // it once. When the link carries ?open (a Week chip or search row jumping to
    // an expanded entry) the view layer scrolls that entry into view after
    // render — don't yank it back to the top.
